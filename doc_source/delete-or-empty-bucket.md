...
For more information, see [Object Lifecycle Management](object-lifecycle-mgmt.md) and [Expiring Objects: General Considerations](lifecycle-expire-general-considerations.md)\. 
...
You can configure lifecycle on you bucket to expire objects and request that Amazon S3 delete expired objects\. You can add lifecycle configuration rules to expire all or a subset of objects with a specific key name prefix\. For example, to remove all objects in a bucket, you can set lifecycle rule to expire objects one day after creation\.
