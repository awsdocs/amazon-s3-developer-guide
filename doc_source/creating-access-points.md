...
For more information about using condition keys with Amazon S3, see [Specifying Conditions in a Policy](amazon-s3-policy-keys.md)\.
...
The following access point policy grants IAM user `Alice` in account `123456789012` permissions to `GET` and `PUT` objects with the prefix `Alice/` through access point `my-access-point`\.  
...
**Note**  
For the access point policy to effectively grant access to `Alice`, the underlying bucket must also allow the same access to `Alice`\. The following bucket policy grants the necessary permissions to Alice\.  
...
The following access point policy grants IAM user `Bob` in account `123456789012` permissions to `GET` and `PUT` objects through access point `my-access-point` that have the tag key `data` set with a value of `finance`\.  
...
```
For the access point policy to effectively grant access to `Bob`, the underlying bucket must also allow the same access to `Bob`\. The following bucket policy grants the necessary permissions to `Bob`\.  
