# Additional Replication Configuration: Replicating Objects Created with Server\-Side Encryption \(SSE\) Using Encryption Keys stored in AWS KMS<a name="replication-config-for-kms-objects"></a>
...
The AWS KMS key *must* be valid\. The `PUT` Bucket replication API doesn't check the validity of AWS KMS keys\. If you use an invalid key, you receive the 200 OK status code in response, but replication fails\.
...
To request an increase in your AWS KMS API rate limit, contact [AWS Support](https://console.aws.amazon.com/support/home#/)\.